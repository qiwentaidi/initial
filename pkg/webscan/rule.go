...
func (ic *InitConfig) InitFingprintDB(fingerprintFile string) error {
...
func boolEval(expression string) bool {
	...
	if strings.Count(expression, "(") != strings.Count(expression, ")") {
		gologger.Warning().Msg(fmt.Sprintf("纯布尔表达式 [%s] 左右括号不匹配", expression))
	}
	// 去除空格
...
func (ic *InitConfig) InitAll(webfinger, activefinger, template string) error {
	FingerprintDB = nil
	if err := ic.InitFingprintDB(webfinger); err != nil {
		return err
	}
